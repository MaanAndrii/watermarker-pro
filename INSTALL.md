...
### Якщо git ще не встановлено / проєкту немає на Pi

Одна команда — завантажує лише скрипт і запускає візард, який сам клонує проєкт:

```bash
curl -fsSL https://raw.githubusercontent.com/MaanAndrii/watermarker-pro/main/install.sh | bash
```

### Якщо проєкт вже склонований
...
Візард крок за кроком (7 кроків) запитає:
1. Перевірка системи (Python, RAM, місце)
2. Клонування з GitHub — URL, гілка, директорія *(або пропустити, якщо файли вже є)*
3. Директорія проєкту, порт, доступ з мережі
4. Встановлення системних пакетів (apt)
5. Встановлення Python-залежностей (venv + pip)
6. Кількість потоків обробки
