...
Три команди — надійно, просто, без жодних хитрощів:
...
# 2. Клонувати проєкт
git clone https://github.com/MaanAndrii/watermarker-pro.git ~/watermarker-pro

# 3. Запустити візард
bash ~/watermarker-pro/install.sh
```

Візард запитає крок за кроком:
1. Перевірка системи (Python, RAM, місце)
2. Підтвердження директорії, порт, доступ з мережі
3. Встановлення системних пакетів (apt)
4. Встановлення Python-залежностей (venv + pip)
5. Кількість потоків обробки
